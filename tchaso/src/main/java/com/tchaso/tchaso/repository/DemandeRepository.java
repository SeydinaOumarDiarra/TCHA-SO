...
public interface DemandeRepository extends JpaRepository<Demande, Integer> {
    ...
    @Modifying
    @Query(value="SELECT d FROM Demande d WHERE d.client.id =:id ORDER BY (d.id) DESC")
    public List<Demande> AllNotifyClient(@Param("id") Integer Id);

    @Modifying
    @Query(value="SELECT d FROM Demande d WHERE d.travailleur.id =:id ORDER BY (d.id) DESC")
